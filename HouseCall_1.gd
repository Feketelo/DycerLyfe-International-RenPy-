...
onready var text = get_parent().get_node("Dialogue").encounter_1
...
func _physics_process(delta):
	if active:
		if Input.is_action_just_pressed("ui_accept"):
			$TextBox/RichTextLabel.bbcode_text = "CLICK"
			if finished == true:
				load_dialogue()
			else:
...
func nextDialogue(index, words):
	...
	if dialogue_index < text[dialogue_index]["Choices"].size() and text[dialogue_index]["Choices"][index]["GoTo"]:
		dialogue_index = text[dialogue_index]["Choices"][index]["GoTo"] - 1
	else:
		dialogue_index +=1
	load_dialogue()
